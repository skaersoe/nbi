...
class CycleCreator:
    ...
    def ReadVariableSelection(self,filename):
        ...
        try:
            ...
            print "Unable to open variable selection file","\""+filename+"\""
            return varlist
    ...
    class ROOT_Access:
        ...
        def ReadVars(self, rootfile, treename):
            ...
            if not rootfile or not treename:
                print "Incomplete arguments. Cannot get tree named","\""+treename+"\"","from rootfile","\""+rootfile+"\""
                return varlist
            
            f=ROOT.TFile.Open(rootfile)
            if not f:
                print "Could not open root file","\""+rootfile+"\""
                return varlist
            
            tree=f.Get(treename)
            if not tree:
                print "Could not get tree","\""+treename+"\""
                f.Close()
                return varlist
    ...
    def Backup(self,filename):
        ...
        if os.path.exists(filename):
            ...
            print "WARNING:: Moving \"" + filename + "\" to \"" + \
                  filename + ".backup\""
            import shutil
    ...
    def CreateSource( self, className, fileName = "", namespace="", varlist=[], create_output=False):
        # Construct the file name if it has not been specified:
        if fileName == "":
        ...
        if namespace:
            ...
            ns_body=body
        full_contents=self._Template_source_Frame % {"body":ns_body, "fullClassName":fullClassName,"header":self._headerFile}
    ...
    def CreateCycle( self, cycleName, linkdef = "", rootfile = "", treename = "", varlist = "", outtree="", analysis=""):
        ...
        if not analysis:
            ...
            print "Using analysis name","\""+analysis+"\""
